...
function fbfull_feedback()
{
	...
	<textarea id=\"fbfull_send_response\" cols=\"70\"></textarea><br>
	...
	<thead>
		<tr>
			<th>
				".__('News','fbfull')."<div style=\"float: right;\"><a wait_text=\"".__('Please wait...','fbfull')."\" all_text=\"".__('All news by author','fbfull')."\" plugin_text=\"".__('Only plugin news','fbfull')."\" type=\"plugin\" id=\"fbfull_type_news\" href=\"#\">".__('All news by author','fbfull')."</a></duv>
			</th>
		</tr>
	</thead>
	<tbody id=\"news_list\">
...
if( is_admin() )
{
	...
	function fbfull_admin_main_add_js_css()
	{
		...
		wp_enqueue_script('fbfull-admin', get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/js/fbfull_admin.js", array('jquery'));
	...
	add_action('wp_ajax_change_visible_comments', 'change_visible_comments_callback');
	function change_visible_comments_callback()	{
		if(get_option('fbfull_pages_comments'))
			$pages_comments = get_option('fbfull_pages_comments');
		else
			$pages_comments = array();
		if(isset($pages_comments[$_POST['page_id']]))
			$page_comments = $pages_comments[$_POST['page_id']];
		else
			$page_comments = array();
		$page_comments['show'] = $_POST['show'];
		$pages_comments[$_POST['page_id']] = $page_comments;
		update_option('fbfull_pages_comments',$pages_comments);
		$response = array();
		if($_POST['show'])
		{
			$response['show'] = "1";
			$response['img'] = "<img title=\"".__('Close Facebook Comments','fbfull')."\" src=\"".get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/images/fbfull_comments.png\">";
		}
		else
		{
			$response['show'] = "0";
			$response['img'] = "<img title=\"".__('Open Facebook Comments','fbfull')."\" src=\"".get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/images/fbfull_no_comments.png\">";
		}
		echo json_encode($response);
	}
	function fbfull_add_media_button()
	{
		global $post;
		...
		if($page_comments = $pages_comments[$post->ID])
		{
		...
		if(($post->post_status == 'publish')||($post->post_status == 'private')||($post->post_status == 'pending'))
		{
			if($show)
			{
				echo "<span><a id=\"fbfull_post_action\" page_id=\"".$post->ID."\" show=\"0\" href=\"#\"><img title=\"".__('Close Facebook Comments','fbfull')."\" src=\"".get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/images/fbfull_comments.png\"></a></span>";
			}
			else
				echo "<span><a id=\"fbfull_post_action\" page_id=\"".$post->ID."\" show=\"1\" href=\"#\"><img title=\"".__('Open Facebook Comments','fbfull')."\" src=\"".get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/images/fbfull_no_comments.png\"></a></span>";
			echo "<script src=\"".get_bloginfo('siteurl'). "/wp-content/plugins/" . dirname(plugin_basename( __FILE__ )) . "/js/fbfull_post.js\"></script>";		
		}
	}
	...
	function fbfull_admin_menu(){
		...
		if(get_option('fbfull_comments_status'))
			add_action('media_buttons','fbfull_add_media_button');
...
else
{
	...
	function fbfull_comments_add($content) {
		...
		if(get_option('fbfull_comments_status'))
		{
			...
			if($pages_comments = get_option('fbfull_pages_comments'))
			{
				if(isset($pages_comments[$post->ID]))
				{
					$page_comments = $pages_comments[$post->ID];
					if($page_comments['show'] == 1)
						$show = 'true';
					else
						$show = 'false';
				}
